...
CoTri. BadNL itself attributes \texttt{bb}/\texttt{cf} to RIPPLES
\cite{chen2021badnl}; a later evaluation instantiates its BadNL baseline with
\texttt{cf} \cite{du2024generated}. BadAgent's OS example appends
\texttt{you know} \cite{wang2024badagent}. CoTri inserts \texttt{tq}
...
Triggerless clean-label \cite{gan2022triggerless} & None & 0 \\
...
The NAACL clean-label attack requires no external inference-time trigger but
targets selected classification examples \cite{gan2022triggerless}. Pure TST
activates from dialogue turn index and MetaBackdoor from sequence length; both
also match our zero-character count \cite{lu2026turn,wen2026metabackdoor}.
Our narrower distinction is the trigger
