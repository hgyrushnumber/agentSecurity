...
\subsection{Trigger conditions and audit information}
Table~\ref{tab:trigger-audit} compares the information needed to evaluate
known trigger conditions in an agent workflow. A lexical marker placed in the
current request is visible there; turn and length conditions can be evaluated
from dialogue structure and input-length metadata, respectively. In contrast,
our intended predicate combines tool identity, execution status, and the first
crossing of a cumulative success threshold. Request text, turn count, and
input length alone do not generally determine this predicate.

\begin{table}[t]
\centering
\caption{Trigger conditions and information for direct auditing of a known
predicate. Rows compare selected mechanisms, not reproduced attacks or
measured detection rates.}
\label{tab:trigger-audit}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.20\columnwidth}p{0.31\columnwidth}p{0.41\columnwidth}@{}}
\toprule
Mechanism & Trigger condition & Audit information \\
\midrule
Lexical \cite{wang2024badagent} & Marker in current request & Current request text \\
Turn \cite{lu2026turn} & Turn-index threshold & Dialogue structure or turn index \\
Length \cite{wen2026metabackdoor} & Input-length threshold & Serialized input token count \\
Ours & First third success for the same tool & Ordered tool identities and execution statuses \\
\bottomrule
\end{tabular}
\end{table}

The lexical row refers to current-request insertion, as in BadAgent's OS
example; it does not describe every lexical or multi-step attack. The turn and
length rows refer to pure TST and length-only MetaBackdoor. Their respective
turn convention, tokenizer, and serialization must be respected when auditing
the condition. Like these two settings, our positive prefixes add no lexical
trigger marker beyond the common policy and sandbox schema. Zero added
markers are therefore a shared property, not our distinguishing contribution.

The distinction is the additional dependence on execution-result information.
For a trigger prefix and its matched final-failure control, an audit projection
that suppresses all result content and status while preserving the remaining
identical history cannot distinguish the pair. This is a conditional property
of that projection, not a measured detection rate. Status may also be conveyed
in tool-result text; real token counts, result sizes, and subsequent behavior
may reveal differences even without an explicit status field. Indistinguishability
under such richer views requires separate verification. With ordered tool
identities and recoverable statuses, an auditor knowing the predicate can
compute it directly. Table~\ref{tab:trigger-audit} characterizes the designed
condition, not whether the model has learned it or whether full-trace auditing
can detect a backdoored model. Behavioral boundary tests follow in
Table~\ref{tab:main}.
