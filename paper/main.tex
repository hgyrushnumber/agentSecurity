...
distributed across a trajectory \cite{qiu2025chaintrigger}. We study a narrower
...
published SFT-backdoor configurations. We count Unicode characters in the
...
BadAgent's OS example appends \texttt{you know}
\cite{wang2024badagent}. Chain-of-Trigger (CoTri) inserts an initial marker,
then uses environment-derived observations for later triggers
\cite{qiu2025chaintrigger}. Our positive prefixes use existing tool events
without adding a lexical marker. The common system policy and sandbox tool
schema are experimental scaffolding, not included in this marker count.

\begin{table}[t]
\centering
\caption{Added lexical trigger text in selected SFT-backdoor settings.
Character counts are computed from published marker literals; zero for ours
is a construction property. The lower panel reports only our local validation
results, not a cross-paper ASR ranking.}
...
This removes the added marker in the listed configurations while retaining
high conditional activation in our selected validation histories. It does not
establish lower poisoning cost, better detector evasion, or superiority to
other trigger families. We have not rerun the external methods on this cohort;
their ASR under zero added text is unmeasured here. Natural trigger prevalence
and end-to-end execution success also remain unmeasured. Marker lengths describe
explicit insertion, not minimum intervention when a marker occurs naturally.
