...
表 1 将前一项量化为“指定设置中额外插入的词汇标记字符数”：RIPPLES 的 `cf`
示例为 2，BadAgent OS 的 `you know` 为 8，CoTri 的 `tq` / `cf` / `exactly` 为
2 / 2 / 7，本方法为 0。`cf` 至少可追溯到 RIPPLES（ACL 2020）；BadNL 原文也把
`bb/cf` 归因于该工作，后续实现才常用“BadNL + cf”这一 baseline 配置。
计数包含标记内部空格，不包含外侧分隔符；不是 tokenizer token 数，也不是总编辑距离。
CoTri 三行是同一方法的不同设置，不当作三个独立方法。Triggerless clean-label、pure
TST 和 length MetaBackdoor 同样为 0 字符，分别依赖目标测试样本、轮次和长度；所以
零字符不是本方法首创或相对所有攻击的优势。本文的可保留差异是零词汇标记条件下使用
工具身份、执行状态和三次累计组成的 predicate。字符比较只支持相对列出的显式词汇设置，
