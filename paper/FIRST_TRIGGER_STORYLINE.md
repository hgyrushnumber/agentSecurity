...
表 1 的主旨改为“触发条件与审计信息需求”：当前请求插词设置可从请求文本判定；
Turn 依赖相应轮次结构，Length 依赖真实序列长度，本文条件还需要有序工具身份与
可恢复的执行状态。这里描述的是所设计的 predicate，不是模型已经学会完整历史规则。
表 2 单独报告行为边界结果，避免把设计属性和已测性能混在同一表中。

零额外词汇标记降为辅助属性：pure TST、length MetaBackdoor 和本文都具备，
不是本方法首创。旧字符比较与 cf 文献追溯保存在 trigger_text_evidence.json，
当前表 1 的来源和边界保存在 trigger_audit_evidence.json。

可保留差异是执行结果可观测性的额外依赖。当前表 1 是定性机制比较，不是检测性能排名，
也不是在统一任务下完成了所有攻击的训练和评测。配对仅在明确投影相同的视图下不可区分；
现有工具删除整个结果，不足以证明仅缺少 status 或保留真实长度元数据时仍不可区分。
状态可写在工具结果文本中，所以不能说完整模型 Prompt 或一切对话文本中都没有证据。
完整有序状态可用且规则已知时可以直接判定条件；这不等于识别模型是否含有后门。

普通工作流会自然满足条件也是双刃剑：必须在正常长历史上评估误触发与任务完成率。
“依赖执行状态”不自动推出更高检测成本、整体更隐蔽或跨方法 ASR 优势。
